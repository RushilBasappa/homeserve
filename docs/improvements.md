...
## 3. 💡 Stack placement, moving stacks, and the orphan/duplicate footgun

**Context.** During the SC-005 test we ended up with two stacks running the same
`whoami` compose — `whoami` on the Mac and `whoami-dell` on the Dell. That was
intentional (two separately *declared* stacks, one container each), but it exposes
a real sharp edge in how Komodo handles **where** a stack runs.

**The footgun.** Komodo does **not** automatically clean up the old placement when
you change things:

- **Moving a stack between nodes** — if you change a stack's `stack.config.server`
  in `stacks.toml`, sync, and deploy, Komodo deploys to the **new** node but
  **leaves the container running on the old node** (an orphan). You end up with the
  app running in two places until you manually `DestroyStack`/stop the old one.
- **Removing a stack** — deleting a `[[stack]]` from git does **not** stop the
  running container, because our ResourceSync runs with `delete = false`. The
  definition disappears from git but the workload keeps running (and drifts from
  "managed == declared").
- **Running one app on N nodes** — there's no single "deploy everywhere"; each node
  is its own declared stack (the `whoami-dell` / `whoami-mac` pattern from research
  R8), which is fine but easy to confuse with a duplicate.

**Options / practices.**

- **Document a "move a stack" runbook step**: retarget in git → sync → deploy to
  the new node → **`DestroyStack` (or stop) on the old node** → confirm one
  instance. Make the teardown an explicit, non-optional step.
- **Decide on `delete` for the ResourceSync.** Turning on `delete = true` makes
  Komodo prune resources not in git (true "managed == declared"), but it's
  sharper — a bad commit could tear down a workload. Consider enabling it only
  with the manual-confirm sync gate, or keep it off and enforce teardown by hand.
- **Naming convention for multi-node apps**: if an app intentionally runs on both
  nodes, always suffix the node (`<app>-dell` / `<app>-mac`) so two containers of
  the same image read as *intended*, not accidental. Add this to
  [`CONVENTIONS.md`](./CONVENTIONS.md).
- **A "one instance" check**: a small conformance script that flags the same
  `container_name`/image running on more than one node unless both are declared.

**Recommendation.** 📌 Add a **"move / decommission a stack"** section to the
conventions + runbook that makes the old-node teardown explicit, and make an
explicit decision on the sync `delete` flag. Low effort, prevents silent
duplicates and orphans as the fleet grows.

**Immediate cleanup.** `whoami-dell` was test scaffolding — it can be destroyed and
removed from `stacks.toml` now that SC-005 is proven (leaving just `whoami` on the
Mac). See the note in `tasks.md` about retiring the whoami test stack at end of
Phase 2.
