...
- [~] T031 [US8] 🔸 **advertised 2026-07-19** — ran `tailscale set --advertise-routes=10.0.0.0/24` on the Dell (ip_forward=1). **Remaining (you):** approve the route in the Tailscale admin console; ensure operator devices `--accept-routes`. Original — Advertise the LAN subnet to Tailscale clients: `tailscale up --advertise-routes=10.0.0.0/24` on the Dell, approve the route in the Tailscale admin, and confirm operator devices `--accept-routes` (research R9; FR-019)
...
- [~] T034 [US8] 🔸 **deployed 2026-07-19** — wg-easy deployed from Core; healthy; `51820/udp` + `51821/tcp` published; admin UI 200 on `:51821`. **Remaining (you):** create a client config in the wg-easy admin (LAN/Tailscale only). Original — Declare the wg-easy stack; deploy from Core; create a client config in the wg-easy admin (LAN/Tailscale only) (data-model §11)
