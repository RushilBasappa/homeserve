...
- [X] T031 [US8] ✅ **verified 2026-07-19** — advertised `10.0.0.0/24` on the Dell; route **approved** in the Tailscale console (`PrimaryRoutes: 10.0.0.0/24`). Original — Advertise the LAN subnet to Tailscale clients: `tailscale up --advertise-routes=10.0.0.0/24` on the Dell, approve the route in the Tailscale admin, and confirm operator devices `--accept-routes` (research R9; FR-019)
- [X] T032 [US8] ✅ **verified 2026-07-19** — UDP 51820 forwarded to the Dell on the xFi gateway; proven by an inbound WireGuard handshake from an external cellular IP (172.59.130.34). Original — **GO path** (per T027): forward **only** UDP 51820 → the Dell on the xFi gateway; document the exact xFi steps in the runbook (FR-015)
...
- [X] T034 [US8] ✅ **verified 2026-07-19** — wg-easy deployed from Core, healthy; a client config was created and used (1 peer with a live handshake). Original — Declare the wg-easy stack; deploy from Core; create a client config in the wg-easy admin (LAN/Tailscale only) (data-model §11)
- [X] T035 [US8] ✅ **verified 2026-07-19** — off-LAN/off-Tailscale phone (cellular endpoint 172.59.130.34) connected via `vpn.ragnaforge.xyz`, live handshake + bidirectional traffic. Original — From a device off-LAN and off-Tailscale (e.g. phone hotspot), import the `.conf`, connect via `vpn.ragnaforge.xyz`, and verify `https://whoami.ragnaforge.xyz` + `https://home.ragnaforge.xyz` load over trusted HTTPS; confirm the same app also loads over Tailscale (quickstart Scenario 6; SC-011/012)
- [X] T036 [US8] ✅ **2026-07-19** — external port scan confirmed by operator: only UDP 51820 reachable on `76.102.108.83`; no app/dashboard/admin surface public. (Agent independently confirmed 51820 IS reachable via the inbound handshake; the "only 51820" exclusivity is per the operator's external scan.) Original — Port-scan the home's public IP from an external vantage and confirm **only UDP 51820** responds — no app, dashboard, resolver, Traefik/AdGuard/wg-easy admin reachable off-LAN (quickstart Scenario 7; SC-010; FR-016)
